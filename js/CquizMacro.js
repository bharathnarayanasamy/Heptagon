...
module.exports.CquizMacro =async ctx =>{
        ctx.session.count=0;
        ctx.session.score=0;
...
module.exports.actions2 =async(ctx,actionName) =>
{
    let res;
    const re=/answercm[0-3]/g;
...
const keyboard=(m,step,answers)=>{
       ...
       {
       return [
           [m.callbackButton(answers[0],'answercm0'),m.callbackButton(answers[1],'answercm1')],
           [m.callbackButton(answers[2],'answercm2'),m.callbackButton(answers[3],'answercm3')]
       ]
    }else if(step===2)
    {
    return [
